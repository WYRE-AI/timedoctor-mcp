...
function startHttpServer(): void {
  ...
  const httpServer = createHttpServer(async (req, res) => {
    ...
    if (url.pathname === '/health') {
      ...
      // mode, credentials arrive per-request via X-TimeDoctor-* headers, not
    ...
    const email = isGatewayMode ? (req.headers['x-timedoctor-email'] as string | undefined) : undefined;
    const password = isGatewayMode ? (req.headers['x-timedoctor-password'] as string | undefined) : undefined;
    ...
    if (email && password) {
      await runWithCredentials({ email, password }, handle);
    } else {
      await handle();
    }
  });
